const quizData = {
  'C++': [
    ...
    {q:'What is the correct include for string type?', options:['#include <string>','#include <iostream>','#include <vector>','#include <sstream>'], answer:'#include <string>'},
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!name || !subject || !quizData[subject]) {
    alert("بيانات الكويز غير صحيحة، سيتم إعادتك للرئيسية.");
    window.location.href = "index.html";
    return;
  }

  const activeQuiz = quizData[subject].slice();
  let currentQ = 0;
  let currentScore = 0;
  ...
  function renderQuestion() {
    ...
    quizOptions.innerHTML = item.options.map((opt) =>
      `<label class="option-label"><input type="radio" name="quizOption" value="${opt.replace(/"/g, '&quot;')}"> ${opt}</label>`
    ).join('');
  ...
  nextQuestion.addEventListener('click', () => {
    ...
    if (selected.value === activeQuiz[currentQ].answer) currentScore++;
    currentQ++;
    if (currentQ < activeQuiz.length) {
      renderQuestion();
    } else {
